...
{
    public class ObjectPoolManager : CoreBehaviour, IObjectPoolManager
    {
        ...
        private void ValidateAllPools()
        {
            foreach (var setup in _poolSetups)
            {
                if (!setup.IsValid())
                {
                    _debugLogger.Error($"Invalid setup: {setup.Prefab?.name ?? "null"} - Not PoolableObject or null");
                    continue;
                }
                ...
                if (poolExists)
                {
                    _debugLogger.System($"{setup.Prefab.name} ({type.Name}) - Pool exists");
                }
                else
                {
                    _debugLogger.Error($"{setup.Prefab.name} ({type.Name}) - Pool not created");
                }
            }
        }
            ...
            where T : PoolableObject
        {
            ...
            _debugLogger.System($"Create ObjectPool for {type.Name}");
            return pool;
        }
        ...
        public T Get<T>() where T : PoolableObject
        {
            ...
            _debugLogger.Error($"No pool found for type {type.Name}");
            return null;
        }
